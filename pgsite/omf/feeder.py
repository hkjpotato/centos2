...
def write(inTree):
	print '!!!!!!!!KJTEST: feeders.write() #Turn an omf.feeder tree object into a GLM-formatted string.!!!!!!!!'
...
def sortedWrite(inTree):
	print 'KJTEST: feeders.sortedWrite() #Write out a GLM from a tree, and order all tree objects by their key. Sometimes Gridlab breaks if you rearrange a GLM.!!!!!!!!'
...
def attachRecorders(tree, recorderType, keyToJoin, valueToJoin):
	...
	# stored the information about object of recoreder&collector in memory, will copy it to the tree structure, making it a new 'object' in it
	...
	# loop through the tree, find the corresponding object(e.g. regulator), use the name of that regulator as the parent, and them add new 'node'
...
def groupSwingKids(tree):
	print '!!!!!!!!KJTEST: feeders.groupSwingKids()!!!!!!!!'
...
def treeToNxGraph(inTree):
	print '!!!!!!!!KJTEST: feeders.treeToNxGraph()!!!!!!!!'
...
def _tokenizeGlm(inputStr, filePath=True):
	print '!!!!!!!!KJTEST: feeders._tokenizeGlm()!!!!!!!!'
